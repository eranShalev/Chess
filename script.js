...
function chessRow(row, index, dimensions) {
    ...
    for (let i = 0; i < dimensions; i++) {
        ...
        tile.addEventListener('click', () => { tileClicked(index, i) });
...
function tileClicked(rowIndex, columnIndex) {
    for (let i = 0; i < 8; i++) {
        for (let j = 0; j < 8; j++) {
            tile = document.getElementById(String(i + "-" + j));
            color = getComputedStyle(tile).borderColor;

            if (color !== "rgb(0, 0, 0)") {
                tile.style.borderColor = "rgb(0, 0, 0)";
            }
        }
    }

    document.getElementById(String(rowIndex + "-" + columnIndex)).style.borderColor = "rgb(255, 0, 0)";
}
